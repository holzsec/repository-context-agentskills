...
def render_heatmap(
    ...
    ax.set_xticklabels(labels, rotation=25, ha="right", fontsize=tick_font_size)
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            ax.text(j, i, format_k(value), ha="center", va="center", color=text_color, fontsize=annotation_font_size)
    ...
    if show_diagonal_text:
        for i, total in enumerate(diagonal_totals):
            ...
            ax.text(i, i, format_k(float(total)), ha="center", va="center", color=text_color, fontsize=annotation_font_size)

    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("Overlap", fontsize=colorbar_font_size)
    cbar.ax.yaxis.set_major_formatter(FuncFormatter(lambda x, _pos: format_k(float(x))))
    cbar.ax.tick_params(labelsize=colorbar_font_size - 1, length=3, width=0.8)

    ax.set_xlabel("")
    ax.set_ylabel("")

    fig.tight_layout()
    fig.subplots_adjust(left=0.26, bottom=0.20, right=0.88, top=0.98)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fig-width", type=float, default=5.2, help="Figure width in inches")
    parser.add_argument("--fig-height", type=float, default=3.9, help="Figure height in inches")
    parser.add_argument("--tick-font-size", type=int, default=15, help="Axis tick label font size")
    parser.add_argument("--annotation-font-size", type=int, default=15, help="Cell annotation font size")
    parser.add_argument("--colorbar-font-size", type=int, default=13, help="Colorbar label/tick font size")
    parser.add_argument(
        "--show-diagonal-text",
        action="store_true",
        help="Annotate diagonal cells with marketplace totals. By default diagonal cells are blank.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    render_heatmap(
        ...
        args.colorbar_font_size,
        args.show_diagonal_text,
    )
