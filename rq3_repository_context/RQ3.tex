...
%We exclude two instances from our analysis: CrawlHub skills, as there is no GitHub repository context available, and skills that reside the repository's root, as this would only allow metadata, but not codebase scores. However, for the set of \~3.5K repositories we only find \~100 that is hosted as separate GitHub repositories. From the remaining set, we randomly sample 3,000 combinations and collect the corresponding repository metadata and full repository clones.
...
%\Cref{fig:metascore_marketplaces} illustrates these differences across marketplaces. 
...
\begin{figure}[t]
  ...
  %  \centering
  %  \includegraphics[width=0.8\linewidth]{figures/metadata_score_cdf.pdf}
  %  \caption{Metadata score from repositories by marketplaces. } 
  %  \Description{The figure compares metadata scores of repositories across different marketplaces.}
    %SkillsDirectory moderation impact visible through higher scores.
  %  \label{fig:metascore_marketplaces}
  %\end{minipage}
  \end{figure}
...
\begin{comment}
  - skills with multi-repo support: 413
  - mean variance: 71.85
  - median variance: 30.5
  - p90 variance: 202.1327
  - mean stddev: 6.2415
  - median stddev: 5.5227
  - p90 stddev: 14.2173
  - mean range: 14.7149
  - median range: 13.5
  - p90 range: 33.75
